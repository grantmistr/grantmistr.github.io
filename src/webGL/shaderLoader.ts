...
async function InjectIncludes(shaderText: string, path: string): Promise<string>
{
    /**
     * <full include path, the index of the loaded text in include chunks array>
     */
    const loadedIncludes: Map<string, number> = new Map<string, number>(null);

    /**
     * which index in include chunks array to use
     */
    const includeMap: Map<string, number> = new Map<string, number>(null);
    
    let includeIndex = 0;

    const regex: RegExp = /^[ \t]*#include\s+"(.+?)"/gm;
    ...
    const includeChunks: string[] = new Array<string>(includeIndex);
    ...
    shaderText = shaderText.replace(regex, (fullMatch: string) =>
    {
